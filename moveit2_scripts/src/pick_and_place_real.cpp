...
public:
  ...
  void execute_trajectory_plan() {
    ...
    // setup_goal_pose_target(+0.336, -0.016, +0.155, -0.009, 1.000, -0.026,
    //                        0.000);
    // plan and execute the trajectory
    setup_joint_value_target(-2.393, -1.75467, -1.5827, -1.3629, 1.47396,
                             -0.86300);
    ...
    // setup_waypoints_target(+0.000, +0.000, -0.025);
    // // plan and execute the trajectory
    ...
    // // setup the cartesian target
    //  setup_waypoints_target(+0.000, +0.000, +0.000);
    // // plan and execute the trajectory
    // RCLCPP_INFO(LOGGER, "Planning Cartesian Trajectory...");
    // plan_trajectory_cartesian();
    // RCLCPP_INFO(LOGGER, "Executing Cartesian Trajectory...");
    ...
    std::vector<double> grip_positions = {
        0.5, 0.52, 0.55, 0.57, 0.59, 0.61, 0.63, 0.65, 0.67}; // Gradual close

    for (double pos : grip_positions) {
      RCLCPP_INFO(LOGGER, "Setting gripper to %.2f", pos);
      setup_joint_value_gripper(pos);
      plan_trajectory_gripper();
      execute_trajectory_gripper();
      std::this_thread::sleep_for(
          std::chrono::milliseconds(300)); // Optional delay
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(2000));
    // RCLCPP_INFO(LOGGER, "Preparing Joint Value Trajectory...");
    // setup_joint_value_target(-2.5341, -1.5621, +0.00, +0.00, +0.00, +0.00);
    ...
    RCLCPP_INFO(LOGGER, "Preparing Joint Value Trajectory...");
    setup_joint_value_target(0.344498, -1.239245, -2.19024, -1.3093, 1.6475,
                             1.8753371);
    // plan and execute the trajectory
    RCLCPP_INFO(LOGGER, "Planning Joint Value Trajectory...");
    plan_trajectory_kinematics();
...
private:
  ...
  void plan_trajectory_gripper() {
    ...
    double velocity_scaling_factor = 0.05;     // 10% speed
    double acceleration_scaling_factor = 0.05; // 10% acceleration

    move_group_gripper_->setMaxVelocityScalingFactor(velocity_scaling_factor);
