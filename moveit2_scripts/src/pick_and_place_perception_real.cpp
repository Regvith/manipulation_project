...
public:
  PickAndPlaceTrajectory(rclcpp::Node::SharedPtr base_node, float x, float y,
    ...
    move_group_robot_->setPlanningTime(10.0);            // More time to plan
    move_group_robot_->setNumPlanningAttempts(10);       // Try multiple plans
    move_group_robot_->setMaxVelocityScalingFactor(0.2); // Safer speed
    move_group_robot_->setMaxAccelerationScalingFactor(
        0.2);                                  // Safer acceleration
    move_group_gripper_->setPlanningTime(5.0); // Less time is usually enough
    ...
    box_pose2.position.z =
        target_z_ - s_thickness_ / 2.0 - object_height_ / 2.0;
  ...
  void execute_trajectory_plan() {
    ...
    setup_goal_pose_target(target_x_ + 0.07, target_y_, target_z_, q.x(), q.y(),
                           q.z(), q.w());
    ...
    setup_waypoints_target(0.00, -0.02, -s_thickness_ / 4.0 - 0.02);
    plan_trajectory_cartesian();
    execute_trajectory_cartesian();
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    // setup_waypoints_target(0.00, 0.00, -0.015);
    // plan_trajectory_cartesian();
    // execute_trajectory_cartesian();
    // std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    ...
    setup_named_pose_manipulator("home");
    plan_trajectory_kinematics();
    execute_trajectory_kinematics();
  }
...
private:
  ...
  void setup_named_pose_manipulator(const std::string &pose_name) {
...
int main(int argc, char **argv) {
  ...
  float grasp_z = object_z + height / 2.0 + object_height / 2.0;
